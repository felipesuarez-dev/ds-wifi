...
# desactivar el dnsmasq del sistema (conflicto de puerto 53)
systemctl disable --now dnsmasq.service >/dev/null 2>&1 || true

# 2. Node
NODE="$(command -v node || true)"
if [ -z "$NODE" ]; then
  NODE="$(ls /home/*/.nvm/versions/node/*/bin/node 2>/dev/null | head -1 || true)"
fi
if [ -z "$NODE" ]; then
...
[ -z "$WIFI_IF" ] && WIFI_IF="$(ls /sys/class/net | grep -E '^wl' | head -1)"
LAN_IF="$(ls /sys/class/net | grep -E '^(enp|eth)' | grep -v '^enp.*docker' | head -1)"
...
mkdir -p "$BASE"/{bin,web/public,config,logs,systemd,sudoers}
...
if [ ! -f "$BASE/config/ap.json" ]; then
  ...
  # inyectar interfaces detectadas
...
# 7. systemd units (con ruta de node real)
...
echo " UI web:   http://$(hostname -I | awk '{print $1}'):3120"
